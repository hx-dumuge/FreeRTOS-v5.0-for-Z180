...
typedef struct QueueDefinition
{
	...
	/*< 指向队列存储区域末尾的字节。一旦分配的字节数超过存储队列项所需的字节数，该字节将用作标记。*/
	...
	/*< 阻塞等待发布到此队列的任务列表。按优先级顺序存储。*/
	xList xTasksWaitingToSend;				/*< List of tasks that are blocked waiting to post onto this queue.  Stored in priority order. */
	/*< 阻塞并等待从此队列读取的任务列表。按优先级顺序存储。*/
	xList xTasksWaitingToReceive;			/*< List of tasks that are blocked waiting to read from this queue.  Stored in priority order. */
	/*< 当前队列中的项目数。 */
	volatile unsigned portBASE_TYPE uxMessagesWaiting;/*< The number of items currently in the queue. */
	/*< 队列的长度定义为它将容纳的项目数，而不是字节数。 */
	unsigned portBASE_TYPE uxLength;		/*< The length of the queue defined as the number of items it will hold, not the number of bytes. */
	/*< 队列将保存的每个项目的大小。 */
	unsigned portBASE_TYPE uxItemSize;		/*< The size of each items that the queue will hold. */
	/*< 存储在队列锁定期间从队列接收（或从队列中移除）的项目数量。当队列未锁定时，设置为 queueUNLOCKED。*/
	signed portBASE_TYPE xRxLock;			/*< Stores the number of items received from the queue (removed from the queue) while the queue was locked.  Set to queueUNLOCKED when the queue is not locked. */
	/*< 存储在队列锁定期间传输到队列（添加到队列）的项目数量。当队列未锁定时，设置为 queueUNLOCKED。*/
 ...
 */
/*
* 此文件中的 xQueueHandle 是一个指向 xQUEUE 结构的指针。
* 为了保持定义私有，API 头文件将其定义为指向 void 的指针。
*/
 ...
 */
/*
* 此处已包含公共函数的原型，因此我们无需包含 API 头文件（因为 xQueueHandle 的定义不同）。
* 这些函数已在 API 头文件中记录。
*/
 ...
 */
/*
* 队列注册表只是内核感知调试器定位队列结构的一种方式。它没有其他用途，因此是一个可选组件。
*/
 ...
 */
/*
* 通过调用 prvLockQueue 解锁队列。锁定队列不会阻止 ISR 向队列添加或移除项目，
* 但会阻止ISR 从队列事件列表中移除任务。如果 ISR 发现队列已锁定，它将增加相应的队列锁定计数，
* 以指示某个任务可能需要解锁。当队列处于解锁状态时，将检查这些锁定计数，并采取适当的操作。
*/
...
/*
* 使用临界区判断队列中是否有数据。
* 如果队列中没有数据，则返回 pdTRUE，否则返回 pdFALSE。
*/
 ...
 */
/*
* 将一个项目复制到队列中，可以是队列的前面，也可以是队列的后面。
*/
 ...
 */
/*
* 用于将队列标记为已锁定的宏。锁定队列可阻止 ISR 访问队列事件列表。
*/
