...
/* 这是信号量（Semaphore）和互斥量（Mutex）API 头文件，专门用来管理任务间的同步与资源互斥 */
...
*  ├─ xSemaphoreTake(xSemaphore, xBlockTime)       // 获取信号量
*  ├─ xSemaphoreTakeRecursive(xMutex, xBlockTime)  // 递归获取互斥量
*  ├─ xSemaphoreAltTake(xSemaphore, xBlockTime)    // 简化版获取信号量
*  ├─ xSemaphoreGive(xSemaphore)                   // 释放信号量
*  ├─ xSemaphoreGiveRecursive(xMutex)              // 递归释放互斥量
*  ├─ xSemaphoreAltGive(xSemaphore)                // 简化版释放信号量
*  ├─ xSemaphoreGiveFromISR(xSemaphore, pxHigherPriorityTaskWoken) // ISR中释放信号量
