...
* task.h
*
* 引用任务的类型。例如，调用 xTaskCreate 通过指针参数返回一个 xTaskHandle 变量，
* 该变量随后可以用作 vTaskDelete 的参数来删除该任务。
*
* \page xTaskHandle xTaskHandle
* \ingroup Tasks
*/
...
* 任务控制API
*  ├─ vTaskDelay                 // 延时
*  ├─ vTaskDelayUntil            // 定时延时
*  ├─ uxTaskPriorityGet          // 获取任务优先级
*  ├─ vTaskPrioritySet           // 设置任务优先级
*  ├─ vTaskSuspend / vTaskResume // 挂起/恢复任务
*  ├─ xTaskResumeFromISR         // ISR中恢复任务
* 调度器控制API
*  ├─ vTaskStartScheduler        // 启动调度器
*  ├─ vTaskEndScheduler          // 结束调度器
*  ├─ vTaskSuspendAll / xTaskResumeAll // 挂起/恢复全部任务
*  ├─ xTaskIsTaskSuspended       // 查询任务是否挂起
* 任务工具API
*  ├─ xTaskGetTickCount          // 获取系统滴答计数
*  ├─ uxTaskGetNumberOfTasks     // 获取任务数量
*  ├─ vTaskList                  // 输出任务列表
*  ├─ vTaskStartTrace / ulTaskEndTrace // 任务跟踪
*  ├─ uxTaskGetStackHighWaterMark// 获取任务栈高水位
*  ├─ vTaskSetApplicationTaskTag // 设置任务钩子
*  ├─ xTaskCallApplicationTaskHook // 调用任务钩子
* 调度器内部接口（仅供内核/移植使用）
*  ├─ vTaskIncrementTick
...
*  ├─ vTaskPriorityInherit / vTaskPriorityDisinherit
*/
...
* task.h
*
* 用于标记关键代码区域起始位置的宏。抢占式上下文切换无法在关键代码区域内进行。
*
* 注意：这可能会改变堆栈（取决于可移植的实现）
* 因此必须谨慎使用！
*
* \page taskENTER_CRITICAL taskENTER_CRITICAL
* \ingroup SchedulerControl
...
* task.h
*
* 用于标记关键代码区域结束的宏。抢占式上下文切换无法在关键区域进行。
*
* 注意：这可能会改变堆栈（取决于可移植的实现）
* 因此必须谨慎使用！
*
 ...
 */
/*
* 此函数不得在应用程序代码中使用。它仅旨在用于实现调度器的端口，并且是调度器专用的接口。
*
* 将指向当前 TCB 的指针设置为最高优先级任务的 TCB，即已准备好运行的任务。
*/
